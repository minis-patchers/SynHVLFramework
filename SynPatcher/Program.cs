...
public class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        return await SynthesisPipeline.Instance
            ...
            .SetTypicalOpen(GameRelease.SkyrimSE, "VLRP.esp")
            .Run(args);
    }
    public static void PatchRace(Race newVL, IRaceGetter vanillaVL, IRaceGetter vampireRace)
    {
        ...
        newVL.ArmorRace.SetTo(vampireRace.MorphRace);
    ...
    {
        List<RaceConf> races = [];
        IKeywordGetter keyword = state.LoadOrder.PriorityOrder.Keyword().WinningOverrides().Where(x => x.EditorID == "VampireLord").FirstOrDefault(state.PatchMod.Keywords.AddNew("VampireLord"));
        var vanillaVL = Dawnguard.Race.DLC1VampireBeastRace.Resolve(state.LinkCache);
        var vampireRaces = state.LoadOrder.PriorityOrder.Race().WinningOverrides().Where(x => x.EditorID != "DLC1VampireBeastRace" && !x.EditorID!.Contains("VampireLord")).Where(x => !Config.ignoredMods.Contains(x.FormKey.ModKey)).Where(x => x.HasKeyword("Vampire", state.LinkCache)).ToHashSet();
        foreach (var vampireRace in vampireRaces.Where(x => !x.HasKeyword("VampireLord", state.LinkCache)))
        {
            if (!vampireRaces.Where(x => x.EditorID == $"{vampireRace.EditorID}Lord" || (x.ArmorRace == vampireRace && x.MorphRace == vampireRace)).Any())
            {
                ...
                if (newVL.Keywords != null)
                {
                    newVL.Keywords.Add(keyword);
                }
            }
        }
        var txt = JsonConvert.SerializeObject(races);
        var path = Path.Join(state.DataFolderPath, "VLRP");
